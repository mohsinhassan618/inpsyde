...
if(!class_exists('inpsyde_task_plugin')){

    class inpsyde_task_plugin {
        ...
        private $cache_api_response = false;
        public  $inpsyde_endpoint = false;
        ...
        private function __construct()
        {
            ...
            $this->plugin_name_space    = 'inpsyde';
        ...
        {
            $this->inpsyde_endpoint = get_query_var('inpsyde');
            if( $this->inpsyde_endpoint )
            {
        ...
        public function register_rest_route(){
            register_rest_route( $this->plugin_name_space .'/v1', '/users', array(
                'methods' => 'GET',
                'callback' => array($this,'inpsyde_rest_route_callback_users'),
            ) );

            register_rest_route( $this->plugin_name_space .'/v1', '/users/(?P<id>[\d]+)', array(
                'methods' => 'GET',
                'callback' => array($this,'inpsyde_rest_route_callback_users'),
            ) );
        }
        ...
        public function inpsyde_rest_route_callback_users($data = null){
            ...
            $this->cache_api_response = apply_filters('inpsyde_cache_api_response',true);

            $single_id = isset($data['id']) ? $data['id'] : null;
            $json = get_transient('typicode_api_response');
            $response_code = 0;

            if( ($json != false) && is_array($json) && $this->cache_api_response ){

                if($single_id){

                    $single_result = $this->sort_single_user($json,$single_id);
                    ($single_result != false) ? wp_send_json_success($single_result) :
            ...
            try {
                ...
                $json          = false;
                $response      = wp_remote_get( $this->API,array( 'timeout'=> 20) );
                $response_code = wp_remote_retrieve_response_code( $response );

                if( !is_wp_error( $response ) && $response_code == 200 ){
                    $body = wp_remote_retrieve_body( $response );
                    $json = json_decode($body);
                    if($this->cache_api_response)set_transient('typicode_api_response',$json,12 * HOUR_IN_SECONDS);
    ...
    $inpsyde_task_plugin = inpsyde_task_plugin::get_instance();
